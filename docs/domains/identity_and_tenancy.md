...
`GET /api/v1/auth/me` 每次从当前用户权威状态重新生成资料，其中余额和安全状态都可能在会话期间变化；响应使用 `private, no-store`，前端刷新也要绕过中间缓存，不能依赖退出后重新登录来取得最新值。登录页的公开设置由应用级 Store 共享，已有缓存时应立即开放表单，后台登出吊销或重复设置请求不得阻塞输入。
