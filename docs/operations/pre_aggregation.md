...
管理端分组页的累计、今日和昨日实际费用复用同一组合查询源：累计窗口可使用日表主体，今日与昨日按调用方时区的自然日边界组合小时桶和原始首尾。覆盖不足或聚合查询失败时整项回退到 `usage_logs`，不会再创建一套与通用 Usage 聚合重复的分组专用表。

趋势查询先组合 UTC 桶，再按配置时区重新分桶，因此可处理非 UTC 时区和夏令时切换。最近五分钟 RPM/TPM 始终读取原始记录，避免聚合延迟污染实时指标。

管理员用户消费排行无论由组合聚合源还是原始表回答，都先按 `user_id` 汇总，再在查询时关联 `users` 表读取当前 `username` 与 `email`；身份字段不固化进可重建聚合表。管理端依次用非空用户名、非空邮箱和用户 ID 作为排行标签。
