...
迁移 `254_invoice_requests.sql` 新增人工发票申请、订单快照、附件元数据和邮件投递记录。它只新增表、索引和约束，可按常规前向迁移执行；升级后应抽样验证余额与订阅订单的合并申请、审批、驳回释放订单占用、附件下载及退款限制。附件文件不在 PostgreSQL 中，保存在 `DATA_DIR/invoice-attachments/`（默认容器路径为 `/app/data/invoice-attachments/`）：所有生产部署必须为该目录提供持久且可备份的共享卷。多实例部署若不共享该卷，会导致其它实例无法下载或寄送附件。

迁移 `255_add_invoice_type.sql` 为已部署的发票申请增加 `PERSONAL`/`ENTERPRISE` 类型快照，并把存量记录回填为个人类型。它是兼容的新增非空默认列；升级后应验证企业申请缺失税号会被拒绝、个人申请不保存税号，以及用户和管理员列表均显示冻结的发票类型。

迁移 `256_add_invoice_enterprise_bank_info.sql` 为企业发票申请增加可空的开户行和银行账号字段。字段仅作为开票补充资料，不改变企业抬头和税号的必填规则；升级后应抽样验证个人申请不会写入企业字段，企业申请可正常保存并在管理员详情中查看。

迁移 `257_dashboard_ads_table.sql` 新增 `dashboard_ads` 广告表，并从旧 `settings.dashboard_ads` JSON 尽力回填，成功后删除旧键。广告通过独立管理接口整体替换，普通系统设置保存不会覆盖广告；升级后应验证旧广告回填、广告页保存后刷新仍存在，以及用户公开设置能读取有效顺序。回退旧二进制需恢复数据库备份，不能依赖已删除的旧键。
...
内置长上下文阶梯不再由代码按模型名补齐，改由价格目录数据驱动：条目的 `*_above_<N>k_tokens` 绝对价档在加载时折算为阈值与倍率（阈值取自字段名，倍率 = above 价 ÷ 基础价，多阈值取最小），条目显式声明任一 `long_context_*` 字段（含显式 0）时以显式值为准。因此 `gpt-6-luna`、`gpt-6-sol` 这类只存在于远程价卡、没有代码级兜底的条目也会自动获得阶梯。

升级后会有一批模型开始按内置阶梯结算，而此前只收基础价：`claude-sonnet-4-5`、`claude-4-sonnet-*`、`gemini-2.5-pro`、`gemini-3.x-pro-*`、`gemini-pro-latest` 等超过 200k 输入后按输入 2 倍、输出 1.5 倍，`gpt-6` 系条目超过 272k 后同样按 2 倍、1.5 倍。`gpt-5.4`/`gpt-5.5`/`gpt-5.6` 系此前已由代码兜底，口径不变。这是预期的计费口径扩大，不是配置错误。同一版本还按业务倍率改写 Fast/priority 档价（`gpt-5.5` 为 2.5 倍，其余支持 Fast 的 `gpt-5.4`/`gpt-5.6`/`gpt-6` 条目为 2 倍），修正目录中 `gpt-5.5` priority 仍标 2 倍的少计费。
