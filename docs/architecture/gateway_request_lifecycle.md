...
- [生图历史旁路](#生图历史旁路)：修改最终图片捕获和异步转存时读取。
...
- 普通 Key 先按 `group_ids` 顺序选择首个启用且用户可访问的候选；列表为空时使用入口默认分组。`routing_strategy` 决定最终分组内保持手动调度或启用智能账号排序。复合 Key 必须先用客户端的完整 `前缀/模型` 选择分组，Key 级模型重定向再处理去前缀后的模型。
...
4. 普通 Key 从有序候选中选择首个启用且用户可访问的分组；若为复合 Key，则从请求模型选中映射并得到本次请求的普通 Key 视图。随后校验最终分组，并应用 Key 级模型重定向。
...
账号选择的输入至少包含本次分组、平台、请求模型、渠道解析结果、会话 hash、已失败账号集合、端点能力和有效 `routing_strategy`。先解析 Claude Code-only 等分组回退，再以最终目标分组的 `scheduler_type` 和 Key/请求覆盖选择基础或高级调度器；无分组路径固定使用基础调度器。请求解析会把 `provider.sort` 以及 `:nitro`、`:floor` 模型后缀转换为仅本次有效的策略，并在转发前移除扩展。选择器综合以下约束：
...
只有部署者启用 `image_history` 且当前 API Key 所属用户主动开启保存时，OpenAI Images 与 Grok 图片生成/编辑 handler 才为本次请求附加捕获器。普通 JSON、SSE 和 OpenAI OAuth Images 转换层只收集最终图片，忽略 partial image 并在请求内去重；请求完成后才把图片引用及用户、Key、请求、endpoint、模型和 prompt 元数据提交给独立有界 worker pool。

该旁路不进入账号调度、故障转移或结算幂等事务。队列提交是非阻塞的，队列满、下载、校验、S3 或元数据写入失败都不能改变已经形成的协议响应和费用；失败通过 `image_history.save_dropped_queue_full`、`image_history.save_failed` 或 panic 恢复日志观测。对象存储和用户访问契约见[生图历史](../domains/image_history.md)。
