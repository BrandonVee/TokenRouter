...
登录后的普通业务路由在根 `RouterView` 外共享同一个 `AppLayout`，Header、Sidebar 和 onboarding 生命周期不会因业务页面切换而重建。登录、注册、验证和 OAuth 回调路由同样共享 `AuthShell`，使 Lottie 背景不随认证步骤重载；页面自己的 `AuthLayout` 只负责品牌卡片和 footer 组合。需要独立全屏形态的页面通过路由元数据自行管理布局；公开模型与支付回跳页面继续按各自认证和窗口上下文选择外壳。
...
4. 创建 Redis 客户端、仓储、服务、handler、中间件和 Gin server。多个 provider 会在构造后立即启动各自 worker，例如 token 刷新、到期处理、调度快照、用量记录、聚合、清理、备份、批量图片作业、生图历史转存和支付订单过期处理。
...
| 存储 | 所有权与使用方式 | 失败或丢失影响 |
| --- | --- | --- |
| PostgreSQL | 用户、身份、团队、Key、分组、渠道、账号、设置、订单、订阅、持久任务、生图历史元数据、用量和审计的权威状态 | 连接、迁移或密钥初始化失败会阻止完整应用启动；写失败不得由缓存结果伪装成成功 |
...
| S3 兼容存储 | 备份等可选大对象，以及用户主动开启后的私有生图历史字节 | 备份与生图历史使用独立配置和前缀；历史 S3 失败不影响生图响应，但对象可用性与 PostgreSQL 元数据生命周期必须协同 |
...
- 每个启用生图历史的实例都有独立有界内存队列。任务不跨实例协调且不持久化，队列满或实例关闭可丢弃尚未转存的旁路任务；它不能被当作需要最终完成保证的作业系统。
