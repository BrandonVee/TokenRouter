...
市场接口可以附加 Group capacity 和历史 availability summary。容量来自当前账号/并发投影。主动模式只查询已启用 probe 的 Group；被动模式则为所有合格公开 Group 统计最近 300 次有效请求，并停止后台主动探测 runner 的任务领取。有效样本只包含成功请求和有明确上游归属或上游上下文的失败；业务限制、`count_tokens`、纯客户端错误和本地未知错误不进入分母。健康分中成功记 1 分、上游压力记 0.5 分、明确上游故障记 0 分，少于 10 个有效样本时不展示百分比。前端固定展示 60 根柱，每根汇总连续 5 次请求；单个孤立上游故障降级为黄色，同组至少两次明确故障才显示红色。分组已保存的主动探测配置在被动模式下保留，切回主动模式后恢复生效。用户侧模型广场和 API Key 分组选择器不展示 capacity，管理员分组管理仍正常展示。两者都是辅助信息：读取失败时仍返回模型和价格，不把缺失观测解释为 0 容量或 0% 可用。
