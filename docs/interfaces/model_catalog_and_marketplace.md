...
`context_intervals` 是实际用户定价合同。渠道配置有效 token 区间时原样按区间展示和结算，并优先于模型内置长上下文规则；分组关闭 `long_context_pricing_enabled` 也不会压平这类显式区间。没有渠道区间而模型含长上下文元数据时，开关开启会合成基础档和长上下文档，关闭则展示并结算单一基础价。合成长上下文档时，普通与 priority/Fast 的缓存创建、缓存读取价格都按输入侧倍率调整；缓存创建的标准、5 分钟和 1 小时价格来源保持相同倍率语义。区间价格已经应用 Group 倍率，因此相同请求在任何最终路由账号上都必须得到相同 `ActualCost`；账号配置不能改变公开价格，也不能再次加价。DeepSeek 默认价卡采用官方峰谷价：工作日 UTC `01:00-04:00`、`06:00-10:00` 为 2 倍，北京时间周末全天为 1 倍；分组或渠道自定义价格不叠加该倍率。
...
市场接口可以附加 Group capacity 和历史 availability summary。容量来自当前账号/并发投影。主动模式只查询已启用 probe 的 Group；被动模式固定返回最近 60 个时间桶，每桶宽度由 `marketplace_availability_bucket_minutes` 决定，并停止后台主动测试 runner 的任务领取。有效样本只包含成功请求和明确上游 5xx 错误，以及明确的 `Upstream request failed` 传输错；`429/529` 和限流、容量、并发压力关键词会直接排除。参数错误、路由不可用、认证失败、业务限制、`count_tokens`和本地未知错误不进入分母。主动探测结果中的本地失败也不进入分母，历史结果按错误消息兼容识别明确的上游 5xx/传输失败。每个时间桶用加权异常分判定颜色：明确上游故障每次权重 1.0，非图片流式首字超过 30 秒每次权重 0.25，异常分为两者加权总和除以有效请求数。异常分低于 25% 显示绿色，25% 至 60% 显示黄色；异常分至少 60%、时间桶至少 5 个有效请求且至少 3 次明确上游故障时显示红色。空时间桶按成功计入总体可用率并显示绿色；图片请求不参与慢首字权重，明确上游 5xx 仍然计入故障权重。分组已保存的主动探测配置在被动模式下保留，切回主动模式后恢复生效。用户侧模型广场和 API Key 分组选择器不展示 capacity，管理员分组管理仍正常展示。两者都是辅助信息：读取失败时仍返回模型和价格，不把缺失观测解释为 0 容量或 0% 可用。
