...
市场接口可以附加 Group capacity 和历史 availability summary。容量来自当前账号/并发投影。主动模式只查询已启用 probe 的 Group；被动模式固定返回最近 60 个时间桶，每桶宽度由 `marketplace_availability_bucket_minutes` 决定，并停止后台主动测试 runner 的任务领取。有效样本只包含成功请求和明确上游 5xx 错误，以及明确的 `Upstream request failed` 传输错；`429/529` 和限流、容量、并发压力关键词会直接排除。参数错误、路由不可用、认证失败、业务限制、`count_tokens`和本地未知错误不进入分母。每个时间桶用加权异常分判定颜色：明确上游故障每次权重 1.0，非图片流式首字超过 30 秒每次权重 0.25，异常分为两者加权总和除以有效请求数。异常分低于 25% 显示绿色，25% 至 60% 显示黄色；异常分至少 60%、时间桶至少 5 个有效请求且至少 3 次明确上游故障时显示红色。未知或无数据按绿色展示；图片请求不参与慢首字权重，明确上游 5xx 仍然计入故障权重。分组已保存的主动探测配置在被动模式下保留，切回主动模式后恢复生效。用户侧模型广场和 API Key 分组选择器不展示 capacity，管理员分组管理仍正常展示。两者都是辅助信息：读取失败时仍返回模型和价格，不把缺失观测解释为 0 容量或 0% 可用。
