...
用户主动开启生图历史且部署者已配置独立私有 S3 时，Grok 图片生成和编辑 handler 会从成功的最终 JSON 响应旁路捕获 base64 或临时图片 URL。捕获发生在媒体资格、账号选择、空结果检查和结算完成路径中；视频响应和状态查询不进入图片历史。捕获结果通过与 OpenAI Images 相同的有界后台队列转存，失败不会改变 Grok 原响应、账号健康或计费。完整规则见[生图历史](../domains/image_history.md)。
...
管理员可通过账号创建或更新 API 的 `extra.grok_media_eligible` 覆盖自动判定：`false` 表示排除，`true` 表示强制允许；更新时传 `null` 可删除覆盖并恢复基于探测结果的自动判定，省略该字段则保留现有覆盖。管理端编辑器通过 `GET|PUT /api/v1/admin/accounts/:id/grok-media-eligibility` 读取或设置 `auto/enabled/disabled` 模式；专用更新只增量修改这一扩展键，不能覆盖其它账号运行状态。仅出现每周用量周期不能作为付费层级证据。图片接口返回成功时必须包含至少一个实际图片输出；空的 HTTP `200` 响应会触发账号 failover，不会作为成功生成结果计数或返回。
