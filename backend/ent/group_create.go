...
func (_c *GroupCreate) AddAPIKeyCompositeGroupIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddAPIKeyCompositeGroupIDs(ids...)
	return _c
}

// AddAPIKeyCompositeGroups adds the "api_key_composite_groups" edges to the APIKeyCompositeGroup entity.
func (_c *GroupCreate) AddAPIKeyCompositeGroups(v ...*APIKeyCompositeGroup) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddAPIKeyCompositeGroupIDs(ids...)
}
...
func (_c *GroupCreate) AddDisabledPublicUserIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddDisabledPublicUserIDs(ids...)
	return _c
}

// AddDisabledPublicUsers adds the "disabled_public_users" edges to the User entity.
func (_c *GroupCreate) AddDisabledPublicUsers(v ...*User) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_c *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.APIKeyCompositeGroupsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := _c.mutation.DisabledPublicUsersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
		...
		createE := &UserDisabledPublicGroupCreate{config: _c.config, mutation: newUserDisabledPublicGroupMutation(_c.config, OpCreate)}
		createE.defaults()
		_, specE := createE.createSpec()
		edge.Target.Fields = specE.Fields
		_spec.Edges = append(_spec.Edges, edge)
