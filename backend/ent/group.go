...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldDisplayBrand:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field display_brand", values[i])
			} else if value.Valid {
				_m.DisplayBrand = value.String
			}
		...
		case group.FieldUnavailableFallbackGroupID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field unavailable_fallback_group_id", values[i])
			} else if value.Valid {
		...
		case group.FieldAvailabilityProbeConfig:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field availability_probe_config", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.AvailabilityProbeConfig); err != nil {
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.UnavailableFallbackGroupID; v != nil {
