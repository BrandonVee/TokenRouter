...
// AccountListItemWithConcurrency 是 lite=1 时使用的紧凑账户列表信封：
// 内嵌 dto.AccountListItem 而非完整 dto.Account，列表载荷不再出现
// groups/account_groups 等大字段（移植上游 3e60c3b86）。
...
func (h *AccountHandler) scoreAdvancedSchedulerPool(ctx context.Context, group *service.Group, accounts []service.Account, loadMap map[int64]*service.AccountLoadInfo) map[int64]AccountSchedulerScore {
	...
	schedulableAccounts := make([]*service.Account, 0, len(accounts))
...
func (h *AccountHandler) fetchAdvancedSchedulerLoadMap(ctx context.Context, accounts []*service.Account) map[int64]*service.AccountLoadInfo {
	...
	if batchLoad, err := h.concurrencyService.GetAccountsLoadBatch(ctx, loadReq); err != nil {
		slog.Warn("advanced_scheduler_score_load_batch_failed", "error", err)
...
func (h *AccountHandler) buildAdvancedAccountSchedulerScores(
	ctx context.Context,
	...
	collectAdvancedAccounts := func(pool []service.Account) {
		for i := range pool {
...
func (h *AccountHandler) listAccountSchedulerScoreFilterPool(
	...
	if err != nil {
		slog.Warn("advanced_scheduler_filter_score_pool_failed", "error", err)
		return nil
	}
	return accounts
}
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	if includeSchedulerScore && len(accounts) > 0 {
		...
		schedulerScores, schedulerGroupScores = h.buildAdvancedAccountSchedulerScores(c.Request.Context(), accounts, schedulerFilterPool)
	}
	...
	for i := range accounts {
		acc := &accounts[i]
		// fork：配额自动暂停状态需要逐账号刷新后再进入响应。
		h.applyQuotaAutoPauseState(c.Request.Context(), acc)
		// 上游 3e60c3b86：lite=1 时用紧凑投影，列表载荷不再携带 groups 等大字段。
