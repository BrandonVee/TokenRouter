...
import (
	...
	pkgerrors "github.com/BrandonVee/TokenRouter/internal/pkg/errors"
...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		for {
			...
			submitForwardUsage := func(usageResult *service.ForwardResult) {
				if usageResult == nil {
					return
				}
				userAgent := c.GetHeader("User-Agent")
				...
				if usageResult.ReasoningEffort == nil {
					usageResult.ReasoningEffort = service.NormalizeClaudeOutputEffort(attemptParsedReq.OutputEffort)
				}
				if usageResult.ReasoningEffort == nil && attemptParsedReq.ThinkingEnabled {
				...
				h.submitUsageRecordTask(c, func(ctx context.Context) {
					if err := h.gatewayService.RecordUsage(ctx, &service.RecordUsageInput{
						...
						ForceCacheBilling:  forceCacheBilling,
						APIKeyService:      h.apiKeyService,
						ChannelUsageFields: attemptChannelMapping.ToUsageFields(reqModel, usageResult.UpstreamModel),
					}); err != nil {
						logger.L().With(
							zap.String("component", "handler.gateway.messages"),
							zap.Int64("user_id", subject.UserID),
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformComposite {
		...
		writeDefaultModelsList(c, service.PlatformComposite, availableModels)
		return
	}
...
func (h *GatewayHandler) compositeAvailableModels(ctx context.Context, groupID *int64) []string {
	...
	for _, platform := range []string{service.PlatformAnthropic, service.PlatformGemini, service.PlatformOpenAI, service.PlatformAntigravity, service.PlatformGrok, service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek} {
		...
		platformModels := service.RequestableModelIDs(resolution.Models)
		...
		for _, model := range platformModels {
			...
			if detected, ok := service.DetectModelPlatform(model); ok && detected != platform {
				continue
			}
...
// mergeModelIDs 按输入顺序合并模型 ID，并忽略空值与重复项。
...
// 这类请求用于 Claude Code 验证 API 连通性，流式/非流式探测都需要拦截。
...
// generateRealisticMsgID 生成 Anthropic 官方格式的消息 ID：msg_01 + 22 位 Base62。
