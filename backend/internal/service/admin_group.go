...
func (s *adminServiceImpl) AdminUpdateAPIKeyGroupID(ctx context.Context, keyID int64, groupID *int64) (*AdminUpdateAPIKeyGroupIDResult, error) {
	...
	if *groupID == 0 {
		...
		if group.IsExclusive {
			...
			if err := s.apiKeyRepo.Update(opCtx, apiKey, APIKeyUpdateFields{GroupID: true, GroupIDs: true}); err != nil {
	...
	if err := s.apiKeyRepo.Update(ctx, apiKey, APIKeyUpdateFields{GroupID: true, GroupIDs: true}); err != nil {
		return nil, fmt.Errorf("update api key: %w", err)
	}
