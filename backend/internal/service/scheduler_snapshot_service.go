	...
	if s.cache != nil {
		...
		if err != nil {
			if isContextDoneError(ctx, err) {
				return nil, useMixed, contextDoneError(ctx, err)
			}
...
func (s *SchedulerSnapshotService) handleBulkAccountEvent(ctx context.Context, payload map[string]any, seen map[batchSeenKey]struct{}) error {
	...
	for _, account := range accounts {
		...
		case PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformQoder, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek:
...
func schedulerSnapshotPlatforms() []string {
	return []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformQoder, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek}
}
...
func (s *SchedulerSnapshotService) rebuildByGroupIDs(ctx context.Context, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	buckets := make([]SchedulerBucket, 0, len(groupIDs)*len(schedulerSnapshotPlatforms())*3)
	for _, platform := range schedulerSnapshotPlatforms() {
