...
func (s *PricingService) downloadPricingData() error {
	...
	// 解析 JSON 并依次叠加 fallback 与 override 两层
	data, customHash, err := s.buildPricingData(body)
	if err != nil {
...
func (s *PricingService) parsePricingData(body []byte) (map[string]*LiteLLMModelPricing, error) {
	...
	for modelName, rawEntry := range rawData {
		...
		// 目录已迁移到 *_above_<N>k_tokens 绝对价字段；条目未显式声明任何 long_context_*
		// 字段时按 above 档折算阶梯，避免新条目因缺少代码级模型名兜底而静默失去阶梯。
...
// deriveLongContextFromAboveTierFields 把价格目录的 *_above_XXXk_tokens 绝对价字段
// 折算成 long_context_* 阈值+倍率：阈值取自字段名，倍率 = above 价 ÷ 基础价。
// 条目显式携带任一 long_context_* 字段（含显式 0）时由调用方跳过折算，以显式配置
// 为准——显式写 threshold=0 或 multiplier=1 均可关闭该模型的阶梯。多个阈值并存时
// 取最小阈值。
// cache_read/cache_creation 的 above 档在计费中统一跟随输入倍率，不单独折算：
// 条目须自带基础 cache 价，缺基础价的 cache above 字段无法参与计费，由
// orphanCacheTierFields 哨兵告警。
...
func warnOrphanCacheTierFields(entries []string) {
	...
	logger.LegacyPrintf("service.pricing", "[Pricing] Warning: %d model(s) carry cache above-tier prices without a base cache price; that cache item bills at $0 until the catalog supplies the base: %s", total, strings.Join(entries, ", "))
}
...
func (s *PricingService) loadPricingData(filePath string) error {
	...
	// 解析 JSON 并依次叠加 fallback 与 override 两层
	pricingData, customHash, err := s.buildPricingData(data)
	if err != nil {
		return fmt.Errorf("parse pricing data: %w", err)
	}
...
func (s *PricingService) reloadCustomPricingLayers() error {
	body, err := s.readReloadSourceCatalog()
	if err != nil {
		// 既无目录缓存也无回退文件时没有可重建的底座，跳过本轮而不刷 WARN。
		if errors.Is(err, errNoReloadSource) {
			logger.LegacyPrintf("service.pricing", "%s", "[Pricing] Custom pricing file reload skipped: no local catalog or fallback file available")
			s.mu.Lock()
	...
	logger.LegacyPrintf("service.pricing", "[Pricing] Custom pricing files changed, reloaded %d models", len(data))
	return nil
}

// errNoReloadSource 表示既没有目录缓存也没有回退文件可供重建。
var errNoReloadSource = errors.New("no local catalog or fallback file")

// readReloadSourceCatalog 返回热重载使用的目录正文。优先使用本地目录缓存；未配置远程时
// 该缓存可能根本不存在，此时回退到 pricing.fallback_file，避免每个周期都告警却没有可
// 重建的底座。
func (s *PricingService) readReloadSourceCatalog() ([]byte, error) {
	pricingFile := s.getPricingFilePath()
	if body, err := os.ReadFile(pricingFile); err == nil {
		return body, nil
	...
	if err != nil {
		...
		return nil, fmt.Errorf("read fallback file failed: %w", err)
	}
	return body, nil
}
...
func warnDroppedLongContextLadders(old, next map[string]*LiteLLMModelPricing) {
	...
	logger.LegacyPrintf("service.pricing", "[Pricing] Warning: long-context ladder dropped for %d model(s) after reload: %s (verify catalog data if unintended)", total, strings.Join(dropped, ", "))
}
