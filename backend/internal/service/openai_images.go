...
const (
	...
	openAIImageMaxDownloadBytes            = 20 << 20 // 单张图片最多下载 20MB
	openAIImageMaxUploadPartSize           = 20 << 20 // 单个 multipart 分片最多上传 20MB
...
// openAIImagesResponsesMainModelValue 独立选择 Responses 主控模型，允许运维在上游模型退役时无需重新编译即可切换。
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) {
			...
			decision := s.applyFailoverSideEffects(upstreamCtx, resp, account, respBody, upstreamModel)
			if decision.ShouldReturnGenericError() {
				return s.handleOpenAIImagesErrorResponse(upstreamCtx, resp, c, account, forwardBody, upstreamModel)
			}
