...
const testBuiltinOAuthClientID = "gemini-cli-test.apps.googleusercontent.com"

func setTestBuiltinOAuthClient(t *testing.T) {
	t.Helper()
	t.Setenv(GeminiCLIOAuthClientIDEnv, testBuiltinOAuthClientID)
...
func TestBuildAuthorizationURL(t *testing.T) {
	...
	checks := []string{
		"response_type=code",
		"client_id=" + testBuiltinOAuthClientID,
		"redirect_uri=",
		"state=test-state",
...
func TestBuildAuthorizationURL_RejectsMissingBuiltinCredentials(t *testing.T) {
...
func TestEffectiveOAuthConfig_GoogleOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantClientID: testBuiltinOAuthClientID,
...
func TestEffectiveOAuthConfig_MissingEnvSecret(t *testing.T) {
	t.Setenv(GeminiCLIOAuthClientIDEnv, testBuiltinOAuthClientID)
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "")

	_, err := EffectiveOAuthConfig(OAuthConfig{}, "code_assist")
	if err == nil {
		t.Fatal("缺少 Gemini CLI OAuth client secret 时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Errorf("错误应指出缺少的环境变量，实际: %v", err)
	}
}
