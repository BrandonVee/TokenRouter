...
const (
	...
	rateLimitCacheTTL         = 30 * 24 * time.Hour // 30 days matches the longest window
...
var (
	...
	// updateRateLimitUsageScript 原子累加所有滚动额度窗口。
...
// ARGV[1] = cost（字符串形式的浮点数）
// ARGV[2] = TTL 秒数
// ARGV[3] = 期望的 schema_version（Go 侧 UserPlatformQuotaCacheSchemaV1）
// ARGV[4] = 脏集成员（空串则不 SADD）
// ARGV[5] = 脏集兜底 TTL 秒
