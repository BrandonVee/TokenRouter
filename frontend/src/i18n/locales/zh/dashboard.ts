...
// API Keys
  keys: {
    ...
    smartRouting: {
      label: '分组智能路由',
      hint: '选择顺序决定分组优先级；智能路由决定组内渠道排序。',
      modeLabel: '路由模式',
      overrideHint: '智能路由请求级可覆盖。',
      candidateGroups: '可选模型分组',
      candidateGroupsHint: '请求先从已选分组中确定可用分组，顺序同时作为回退优先级。',
      channelHint: '在已选模型分组内，根据实时指标选择更合适的渠道。',
      enabled: '已开启',
      disabled: '已关闭',
      manualHint: '按分组优先级和分组默认调度规则选择',
      quickTitle: '分组与路由',
      quickHint: '选择候选模型分组，并设置组内渠道排序策略。',
      quickOrderHint: '编号代表分组优先级',
      shortSmart: '智能',
      requestOverrideHint: 'API 请求中的模型后缀和 provider.sort 会覆盖令牌级智能路由。',
      ...
      priceMode: '价格优先',
      priceHint: '优先选择成本更低的渠道',
      successMode: '成功率优先',
      successHint: '优先选择近期更稳定的渠道',
      addGroup: '添加候选分组',
    ...
    rateLimit1d: '日限额',
    rateLimit7d: '周限额（7天）',
    rateLimit30d: '月限额（30天）',
    rateLimitHint: '设置此密钥在指定时间窗口内的最大消费额。0 = 无限制。',
    rateLimitUsage: '消费用量',
    resetRateLimitUsage: '重置消费用量',
    resetRateLimitTitle: '确认重置消费用量',
    resetRateLimitConfirmMessage: '确定要重置密钥 "{name}" 的消费用量吗？所有时间窗口的已用金额将归零。此操作不可撤销。',
    rateLimitResetSuccess: '消费用量已重置',
    failedToResetRateLimit: '重置消费用量失败',
    resetNow: '即将重置',
    expiration: '密钥有效期',
