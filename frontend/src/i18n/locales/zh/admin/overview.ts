...
backup: {
      ...
      s3: {
        ...
        uploadMode: '备份上传模式',
        uploadModeMultipart: '低内存分片上传',
        uploadModeSpooled: '兼容性暂存上传',
        uploadModeHint: 'Cloudflare R2、AWS S3 建议使用低内存分片上传；兼容性暂存上传适用于不支持分片签名的 S3 服务。',
        testConnection: '测试连接',
      ...
      r2Guide: {
        ...
        step1: {
          ...
          line2: '点击「创建存储桶」，输入名称（如 tokenrouter-backups），选择区域',
          line3: '点击创建完成'
        },
        step2: {
          title: '创建 API 令牌',
...
// Groups Management
    groups: {
      ...
      availabilityProbe: {
        ...
        passiveModeHint: '系统当前使用被动探测，按单柱分钟窗口展示最近 60 个时间桶，不会发送主动探测请求。已保存的主动探测配置会保留，切回主动模式后恢复生效。',
        model: '探测模型',
        selectModel: '选择探测模型',
        interval: '探测间隔（分钟）',
        timeout: '超时时间（秒）',
      ...
      openaiLive: {
        title: 'OpenAI Live',
        allow: '允许访问 Live',
        hint: '启用后，此 OpenAI 分组的 API Key 可以创建并控制 Live 语音会话。默认关闭。运行 TokenRouter 的服务端必须是 Apple Silicon Mac，并安装官方 ChatGPT App；客户端平台不受限制。',
