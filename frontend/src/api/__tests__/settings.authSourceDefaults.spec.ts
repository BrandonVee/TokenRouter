...
describe("admin settings auth source defaults helpers", () => {
  it("builds auth source defaults state from flat settings fields", () => {
    const state = buildAuthSourceDefaultsState({
      ...
      auth_source_default_email_subscriptions: [{ plan_id: 1 }],
      ...
      auth_source_default_linuxdo_subscriptions: [{ plan_id: 2 }],
  ...
  it("appends sanitized nested platform_quotas with non-null values in update payload", () => {
    ...
    expect(emailQuotas.qoder).toEqual({ daily: null, weekly: null, monthly: null });
    expect(emailQuotas.grok).toEqual({ daily: null, weekly: null, monthly: null });
    expect(emailQuotas.kimi).toEqual({ daily: null, weekly: null, monthly: null });
    expect(emailQuotas.zhipu).toEqual({ daily: null, weekly: null, monthly: null });
...
describe("normalizePlatformQuotasMap", () => {
  it("填充缺失的平台为全 null 三档", () => {
    ...
    expect(result.qoder).toEqual({ daily: null, weekly: null, monthly: null });
