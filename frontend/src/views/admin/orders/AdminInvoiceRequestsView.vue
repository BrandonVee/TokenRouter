<template>
  <div class="space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div><h1 class="text-xl font-semibold text-gray-900 dark:text-white">发票管理</h1><p class="mt-1 text-sm text-gray-500 dark:text-gray-400">审批申请、上传开票文件并发送至用户的冻结邮箱。</p></div>
      <button class="btn btn-secondary" :disabled="loading" @click="load"><Icon name="refresh" size="sm" :class="loading ? 'animate-spin' : ''" /></button>
    </div>
    <div class="card p-4"><Select v-model="status" :options="statusOptions" class="w-44" @change="load" /></div>
    <div class="card overflow-hidden">
      <div v-if="loading" class="p-8 text-center text-sm text-gray-500">加载中...</div>
      <div v-else-if="requests.length === 0" class="p-8 text-center text-sm text-gray-500">暂无发票申请</div>
      <table v-else class="w-full text-left text-sm"><thead class="bg-gray-50 text-xs text-gray-500 dark:bg-dark-800"><tr><th class="px-4 py-3">申请编号</th><th class="px-4 py-3">用户</th><th class="px-4 py-3">类型</th><th class="px-4 py-3">抬头</th><th class="px-4 py-3">金额</th><th class="px-4 py-3">状态</th><th class="px-4 py-3"></th></tr></thead><tbody class="divide-y divide-gray-100 dark:divide-dark-700"><tr v-for="request in requests" :key="request.id"><td class="px-4 py-3 font-mono text-xs">{{ request.request_no }}</td><td class="px-4 py-3">#{{ request.user_id }}</td><td class="px-4 py-3">{{ invoiceTypeLabel(request.invoice_type) }}</td><td class="px-4 py-3">{{ request.invoice_title }}</td><td class="px-4 py-3">{{ amount(request.total_amount, request.currency) }}</td><td class="px-4 py-3"><span class="rounded bg-gray-100 px-2 py-1 text-xs dark:bg-dark-700">{{ statusLabel(request.status) }}</span></td><td class="px-4 py-3 text-right"><button class="btn btn-secondary btn-sm" @click="openDetail(request.id)"><Icon name="eye" size="sm" /></button></td></tr></tbody></table>
    </div>
...
async function load() { loading.value = true; try { const response = await adminInvoiceAPI.list({ status: status.value || undefined }); requests.value = response.data.items || [] } catch { appStore.showError('无法加载发票申请') } finally { loading.value = false } }
async function openDetail(id: number) { try { const response = await adminInvoiceAPI.get(id); detail.value = response.data; issueForm.invoiceNumber = ''; issueForm.issuedAt = ''; issueForm.remark = '' } catch { appStore.showError('无法加载发票申请详情') } }
