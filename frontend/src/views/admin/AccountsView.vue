...
type AccountBulkEditFilterSnapshot = Extract<AccountBulkEditTarget, { mode: 'filtered' }>['filters']
...
const load = async (options: AccountLoadOptions = {}) => {
  ...
  // 上游 db15e0090：列表始终请求 lite=1 紧凑响应，详情操作改为按需 getById。
...
const handleBulkRefreshToken = async () => {
  ...
  try {
    ...
    if (result.failed > 0) {
      appStore.showError(t('admin.accounts.bulkActions.partialSuccess', { success: result.success, failed: result.failed }))
      // 失败项保留选中，便于修正凭据后直接重试；服务端未返回明细时保留原选择。
      const failedIds = result.failed_ids?.length
        ? result.failed_ids
        : (result.errors?.map(error => error.account_id) ?? [])
...
const buildBulkEditFilterSnapshot = (): AccountBulkEditFilterSnapshot => {
...
const loadBulkEditFilterMetadata = async (filters: AccountBulkEditFilterSnapshot) => {
  const rows: Account[] = []
  let page = 1
  let total = 0

  while (true) {
    const response = await adminAPI.accounts.list(page, BULK_EDIT_FILTER_METADATA_PAGE_SIZE, {
      ...filters,
      lite: '1'
    })
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `tokenrouter-account-${timestamp}.json`
...
// lite 列表行是紧凑投影，打开测试/统计等详情弹窗前先按需拉取完整账户。
...
const handleAdvancedSchedulerScore = async (a: AccountListItem) => {
  const account = await loadAccountDetails(a)
  if (!account) return
  advancedSchedulerScoreAcc.value = account
  showAdvancedSchedulerScore.value = true
}
const handleInviteReset = async (a: AccountListItem) => {
  const account = await loadAccountDetails(a)
  if (!account) return
  inviteResetAcc.value = account
  showInviteReset.value = true
}
...
onMounted(async () => {
  ...
  load()
  try {
    const [p, g] = await Promise.all([adminAPI.proxies.getAll(), adminAPI.groups.getAllIncludingInactive()])
    proxies.value = p
    groups.value = g
  } catch (error) {
