...
const SelectStub = defineComponent({
  ...
  template: `
    <div v-bind="$attrs" :data-current="modelValue">
      <button
...
describe('EditAccountModal Grok media eligibility', () => {
  beforeEach(() => {
    ...
    getEligibilityMock.mockResolvedValue({
      ...
      mode: 'auto',
      eligible: false,
      reason: 'billing_inconclusive'
    })
    updateEligibilityMock.mockImplementation(async (_id: number, mode: string) => ({
      account_id: 12,
      mode,
      eligible: mode === 'enabled',
      reason: mode === 'enabled' ? 'override_enabled' : 'override_disabled'
    }))
  })
  ...
  it('shows only for Grok OAuth and fills the current decision', async () => {
    ...
    expect(wrapper.get('[data-testid="grok-media-eligibility-mode"]').attributes('data-current')).toBe('auto')
    expect(wrapper.get('[data-testid="grok-media-eligibility-status"]').text()).toContain(
      'billing_inconclusive'
    )
    expect(
      mountModal(account('grok', 'apikey')).find('[data-testid="grok-media-eligibility-card"]').exists()
    ).toBe(false)
    expect(
      mountModal(account('openai', 'oauth')).find('[data-testid="grok-media-eligibility-card"]').exists()
    ).toBe(false)
  })
  ...
  it('updates the dedicated endpoint only when the mode changes', async () => {
    ...
    await wrapper.get('[data-testid="grok-media-eligibility-mode"] [data-mode="enabled"]').trigger('click')
  ...
  it('reports partial-save errors when the dedicated endpoint fails', async () => {
    ...
    const wrapper = mountModal()
    await vi.waitFor(() => expect(getEligibilityMock).toHaveBeenCalled())
    await wrapper.get('[data-testid="grok-media-eligibility-mode"] [data-mode="disabled"]').trigger('click')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')
    await vi.waitFor(() =>
      expect(showErrorMock).toHaveBeenCalledWith('admin.accounts.grokMediaEligibility.partialSave')
    )
