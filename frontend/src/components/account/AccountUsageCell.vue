  ...
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    <template v-else-if="account.platform === 'antigravity' && account.type === 'oauth'">
      ...
      <div v-if="antigravityTierLabel" class="mb-1 flex items-center gap-1">
        ...
        <HelpTooltip
          ...
          :content="t('admin.accounts.ineligibleWarning')"
          placement="bottom"
          width-class="w-80 whitespace-normal break-words"
          class="!ml-0"
        >
          <template #trigger>
            <svg
              class="h-3.5 w-3.5 cursor-help text-red-500"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path
    ...
    <template v-else-if="account.platform === 'gemini'">
      ...
      <div v-if="geminiAuthTypeLabel" class="mb-1 flex items-center gap-1">
        ...
        <!-- Help icon -->
        <!-- 额度说明使用 body 级浮层，短列表和末行场景也不会被表格边界遮挡。 -->
        <HelpTooltip
          trigger="both"
          placement="bottom"
          width-class="w-80 whitespace-normal break-words"
        ...
        >
          <template #trigger>
            <svg
              class="h-3.5 w-3.5 cursor-help text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path
...
// 付费 Grok 账号存在 prepaid_balance 时显示预付余额行。
// 月度已用与限额数字只作为辅助信息，主要进度采用 30 天进度条。
const grokPrepaidMoneyLine = computed(() => {
  const billing = grokBilling.value
  if (!billing) return null
  const prepaid = billing.prepaid_balance
  // 只针对预付余额字段存在的账号渲染，包括余额为零的情况。
