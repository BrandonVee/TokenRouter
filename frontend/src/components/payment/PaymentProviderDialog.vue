            ...
            <button
              ...
              :class="[
                ...
                form.payment_mode === mode.value
                  ...
                  : 'border-gray-300 bg-white text-gray-600 hover:border-gray-400 hover:bg-gray-50 dark:border-dark-600 dark:bg-dark-900/70 dark:text-gray-300 dark:hover:border-dark-500',
              ]"
            >{{ mode.label }}</button>
          </div>
        </div>
...
const paymentModeOptions = computed(() => {
  if (form.provider_key === 'alipay') {
    // 支付宝官方：空字符串表示默认先 precreate 再回退 page.pay，redirect 表示始终打开收银台。
    return [
...
function loadProvider(provider: ProviderInstance) {
  ...
  // 将旧数据或跨服务商写入的 payment_mode 归一化到当前服务商允许的值。
