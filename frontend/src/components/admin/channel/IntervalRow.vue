    ...
    <template v-if="mode === 'token'">
      <div class="grid min-w-0 flex-1 grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-6">
        ...
        <template v-if="enableMultipliers">
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputMultiplier', '输入倍率') }}</label>
          ...
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputMultiplier', '输出倍率') }}</label>
            <input :value="interval.output_multiplier" @input="emitField('output_multiplier', ($event.target as HTMLInputElement).value)"
              type="number" step="any" min="0.000001" class="input mt-0.5 text-xs" />
          </div>
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWriteMultiplier', '缓存写倍率') }}</label>
          ...
          <div>
            <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadMultiplier', '缓存读倍率') }}</label>
